...
async def create_flag_definition(flag_data: DynamicFlagCreate):
    """
    Creates a new dynamic flag definition for the DFC engine.
    This defines the rules and metadata for a flag.
    """
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create flag definition: {e}"
        )
...
async def get_all_flag_definitions():
    """
    Retrieves all dynamic flag definitions currently stored in the DFC engine.
    """
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to retrieve flag definitions: {e}"
        )
...
async def get_flag_definition_by_name(flag_name: str = Path(..., description="Name of the flag definition to retrieve")):
    """
    Retrieves a single dynamic flag definition by its unique name.
    """
...
async def update_flag_definition(
    ...
    """
    Updates an existing dynamic flag definition.
    """
...
async def delete_flag_definition(flag_name: str = Path(..., description="Name of the flag definition to delete")):
    """
    Deletes a dynamic flag definition by its unique name.
    """
...
async def apply_dynamic_flags(input_data: FlagApplicationInput):
    """
    Evaluates all defined dynamic flags against the provided entity metadata
    and returns which flags are active.
    """
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to apply flags: {e}"
        )
